...
每个内容项目只保留一套当前稿件和记录，必要时用版本历史保留旧稿。用户仅要求局部改句时可在对话说明，不强制文件体系。
...
投递记录字段：platform、version、requested_action、authorization_evidence、receipt、public_url、readback、status、remaining。无真实值留空，不生成假 ID。
